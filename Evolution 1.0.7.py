...
next_arrow_img = pygame.image.load('Assets/images/arrow.png')
next_arrow_img.set_colorkey((255,255,255))
dna_img = pygame.image.load('Assets/images/dna.png')
dna_img.set_colorkey((255,255,255))
...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y):
        ...
        self.rect = pygame.Rect(player_img, player_img.y, player_img.get_width(), player_img.get_height())
...
"""
class Dna(pygame.Surface):
    def __init__(self, pos_x, pos_y):
        super().__init__(dna_img.get_size())
        self.spinning = [dna_img]
        self.current_sprite = 0
        self.image = self.spinning[int(self.current_sprite)]
    ...
    def update(self,speed):
        self.current_sprite += speed
        if int(self.current_sprite) >= len(self.spinning):
            self.current_sprite = 0
        self.image = self.spinning[int(self.current_sprite)]"""
...
# dnas
dnas = [
    pygame.Rect(50, 50, dna_img.get_width(), dna_img.get_height())
]
...
while True: # game loop
    ...
    display.blit(background_img,(-scroll[0]-62, -scroll[1]-69)) # background

    for dna in dnas:
        background_img.blit(dna_img, (dna[0], dna[1]))

    # moving sprites (Player)
